...
def execute_file_copy():
    ...
    subprocess.run(["python3", script_path])  # Ändere 'python' zu 'python3'

def main():
    """Hauptfunktion, die die Logik steuert."""
    # Prüfen, ob die .env-Datei existiert
    if os.path.exists(ENV_FILE_PATH):
        print("Eine .env-Datei wurde gefunden.")
        choice = input("Möchten Sie neue Eingaben machen? (ja/nein): ").strip().lower()
        if choice == "ja":
            collect_user_input()
        else:
